							...
							char aaaaRecords[MAXRECORDS][AAAARECORDLEN])
{
	...
	{
		uint32_t addrInt;
		inet_pton(AF_INET, entryAddress, &addrInt);
		addrInt = htonl(addrInt);
		char tmp[100];
		inet_ntop(AF_INET, &addrInt, tmp, sizeof(tmp));
		strcpy(&tmp[strlen(tmp)], ".in-addr.arpa");
		//printf("%ld\n", strlen(tmp));
		//printf("========tmp%s======\n", tmp);
		rv = res_query(tmp, ns_c_in, nsType, answer, sizeof(answer));
		if (rv <= 0)
			return -1;
	}
	else
	{
		rv = res_query(entryAddress, ns_c_in, nsType, answer, sizeof(answer));
		if (rv <= 0)
			return -1;
	}
	ns_msg handle;
